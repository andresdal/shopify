...
async function generateProductData(language, description, price, compare_at_price) {
  ...
  <p><strong><img alt="" src="https://cdn.shopify.com/s/files/1/0836/6918/8941/files/fhipinng.png" width="351" height="73" style="display: block; margin-left: auto; margin-right: auto;"></strong></p>
      ...
      attempt++;
      console.error(`Error generating product data, attempt ${attempt}:`, error);

      if (attempt >= maxRetries) {
        throw new Error('Max retries reached. Could not generate product data.');
      }
    }
  }
}

async function generateProductReviews(product_handle, language, country, product_title, image_urls, shopify_domain) {
  ...
  while(attempt < maxRetries) {
    try {
      ...
      const abbreviated_handle = product_handle.substring(0, 10);
      const fileName = `${shopify_domain}_${abbreviated_handle}_reviews.csv`;
      ...
      if (attempt >= maxRetries) {
        throw new Error('Max retries reached. Could not generate product review data.');
      }
    }
  }
}
